  ...
  const handleManualSend = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        alert(t('errorMessage') + response.data.error);
      }
    } catch (error: any) {
      console.error('Failed to send manual notification:', error);
      alert(t('sendError') + (error.response?.data?.error || error.message));
    } finally {
      setSending(false);
    }
  };
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:gap-4">
        ...
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
          ...
          <Button onClick={fetchDuties} loading={loading} variant="ghost" size="sm" className="sm:size-md">
            {t('refresh')}
          </Button>
        </div>
      </div>
