...
function setupCommands(bot) {
    ...
    bot.start((ctx) => {
        ...
        ctx.reply(`👋 SmartHostel Botiga xush kelibsiz!\n\n` +
    ...
    bot.command('oplata', async (ctx) => {
        ...
        try {
            ...
            const msg = await ctx.reply(`💳 Hostel to'lovini yuborish\n\n` +
    ...
    bot.on('text', async (ctx) => {
        ...
        try {
            if (userState.step === 'waiting_for_school21_nick') {
                ...
                const studentsResponse = await axios_1.default.get(`${BACKEND_URL}/api/students`);
                const students = studentsResponse.data;
                const student = students.find((s) => s.username.toLowerCase() === school21Nick.toLowerCase());
                if (!student) {
                    await ctx.reply(`❌ Siz tizimda topilmadingiz.\n\n` +
                ...
                userStates.set(userId, userState);
                const msg = await ctx.reply(`✅ Topildi!\n\n` +
    ...
    bot.on('callback_query', async (ctx) => {
        ...
        if (data === 'start_oplata') {
            ...
            await ctx.reply(`📋 SmartHostel Buyruqlari:\n\n` +
                `/today - Bugungi navbatchiliklar\n` +
                `/tomorrow - Ertangi navbatchiliklar\n` +
                `/oplata - Hostel to'lovini yuborish\n` +
                `/help - Yordam`, { reply_markup: keyboard });
        }
 ...
 */
async function showPaymentPreview(ctx, bot, userId, userState, fileId, fileType) {
 ...
 */
async function sendPaymentToGroup(ctx, bot, userId, userState) {
    ...
    await axios_1.default.post(`${BACKEND_URL}/api/payments`, {
    ...
    // Send confirmation
    await ctx.reply(`✅ To'lov cheki guruhga yuborildi!\n\n` +
        `📝 Ma'lumotlar:\n` +
        `👤 School21: @${school21Nick}\n` +
        `🏷 Ism: ${fullName}\n` +
        `🏠 Xona: ${room}\n` +
...
//# sourceMappingURL=commands_backup.js.map