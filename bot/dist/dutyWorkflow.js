...
exports.startDutyWorkflow = startDutyWorkflow;
exports.handleNicknameInput = handleNicknameInput;
 ...
 */
async function startDutyWorkflow(ctx, studentId, dutyDate) {
    try {
        const userId = ctx.from?.id;
        if (!userId)
            return;
        // Create session in backend
        const response = await axios_1.default.post(`${BACKEND_URL}/api/bot/session`, {
            studentId,
        ...
        // Ask for nickname
        await ctx.reply(`👋 Hello! I'm starting your duty report for ${dutyDate.toLocaleDateString()}.\n\n` +
 ...
 */
async function handleNicknameInput(ctx, text) {
    const userId = ctx.from?.id;
    if (!userId)
        return;
    const session = activeSessions.get(userId);
    if (!session || session.step !== 'nickname')
        return;
    session.nickname = text.trim();
    session.step = 'photos';
    // Update session in backend
    await axios_1.default.patch(`${BACKEND_URL}/api/bot/session/${session.sessionId}`, {
        step: 'photos',
        nickname: session.nickname,
    });
    await ctx.reply(`✅ Got it! Your nickname is: ${session.nickname}\n\n` +
...
async function handlePhotoUpload(ctx) {
    ...
    try {
        ...
        if (session.photoCount < 3) {
            await ctx.reply(`📸 Photo ${session.photoCount}/3 received!\n\n` +
 ...
 * Handle confirmation
 */
async function handleConfirmation(ctx, text) {
    const userId = ctx.from?.id;
    if (!userId)
        return;
    const session = activeSessions.get(userId);
    ...
    if (response === 'yes') {
        try {
            // Submit to backend
            await axios_1.default.post(`${BACKEND_URL}/api/bot/submit-report`, {
            ...
            await ctx.reply(`🎉 Duty report submitted successfully!\n\n` +
...
//# sourceMappingURL=dutyWorkflow.js.map