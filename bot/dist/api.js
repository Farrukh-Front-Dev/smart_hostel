...
exports.setupNotificationEndpoint = setupNotificationEndpoint;
 ...
 */
function generateSimpleMessage(duties, dateStr) {
 ...
 */
function generateTimedMessage(duties, dateStr) {
    let message = `📅 _${dateStr}_\n`;
    message += `━━━━━━━━━\n`;
    const shifts = [
 ...
 */
function setupNotificationEndpoint(app, bot) {
    ...
    app.post('/api/notify/duties', async (req, res) => {
        try {
            ...
            try {
                const formatResponse = await axios_1.default.get(`${BACKEND_URL}/api/settings/duty_message_format`);
            ...
            try {
                const imageResponse = await axios_1.default.get(`${BACKEND_URL}/api/settings/post_with_image`);
            ...
            if (postWithImage && fs_1.default.existsSync(imagePath)) {
            ...
            try {
                await axios_1.default.patch(`${BACKEND_URL}/api/duties/today/status`, {
                    status: 'posted',
                });
