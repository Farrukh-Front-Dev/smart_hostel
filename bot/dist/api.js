...
function setupNotificationEndpoint(app, bot) {
        ...
        try {
            ...
            for (const floor in duties.byFloor) {
                ...
                if (students.length > 0) {
                    ...
                    message += `${floorNum}-й этаж — ${students.map((s) => s.username || s.name).join(', ')}\n`;
                }
            }
            message += `\n╚══════╝\n\n`;
            ...
            for (const floor in duties.byFloor) {
                ...
                if (students.length > 0) {
                    const floorNum = parseInt(floor);
                    message += `${floorNum}-etaj — ${students.map((s) => s.username || s.name).join(', ')}\n`;
                }
            }
    ...
    app.patch('/api/bot/session/:sessionId', async (req, res) => {
        try {
            ...
            const response = await axios_1.default.patch(`${BACKEND_URL}/api/bot/session/${sessionId}`, req.body);
            res.json(response.data);
        }
        catch (error) {
    ...
    app.post('/api/bot/submit-report', async (req, res) => {
        try {
            const response = await axios_1.default.post(`${BACKEND_URL}/api/bot/submit-report`, req.body);
            res.json(response.data);
        }
        catch (error) {
